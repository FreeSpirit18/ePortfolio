...
namespace ePortfolioAPI.Controllers
{
    ...
    public class Post_TagController : ControllerBase
    {
        ...
        {
            return Ok(await _dbContext.Post_Tags.ToListAsync());
        }

        [HttpGet("{id}")]
        public async Task<ActionResult<List<Post_Tag>>> Get(int id)
        {
            var post_tag = await _dbContext.Post_Tags.FindAsync(id);
            if (post_tag == null)
            {
        ...
        {
            var post_tags = await _dbContext.Post_Tags.ToListAsync();
            int max = 0;
            ...
            post_tag.Id = max + 1;
            _dbContext.Post_Tags.Add(post_tag);
        ...
        {
            var dbPost_Tag = await _dbContext.Post_Tags.FindAsync(req.Id);
            if (dbPost_Tag == null)
                return BadRequest("Post_Tag not found");
        ...
        {
            var dbPost_Tag = await _dbContext.Post_Tags.FindAsync(id);
            if (dbPost_Tag == null)
                return BadRequest("Post_Tag not found");
            _dbContext.Post_Tags.Remove(dbPost_Tag);
            await _dbContext.SaveChangesAsync();
            return Ok(await _dbContext.Post_Tags.ToListAsync());
        }
    }
